...
def convert_examples_to_features(examples, tokenizer, max_seq_length, verbose_logging=False, logger=None):
    max_term_num = max((len(example.term_texts) for example in examples), default=0)
    ...
    for (example_index, example) in enumerate(examples):
        ...
        start_indexes, end_indexes, polarity_labels = [], [], []
        for tok_start_position, tok_end_position, polarity in zip(tok_start_positions, tok_end_positions, example.polarities):
            if (tok_start_position >= 0 and tok_end_position + 1 in token_to_orig_map
                    and tok_start_position + 1 in token_to_orig_map):
                ...
                polarity_labels.append(label_to_id[polarity])
...
def convert_absa_data(dataset, verbose_logging=False, keep_empty=False):
    examples = []
